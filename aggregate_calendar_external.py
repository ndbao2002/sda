...
for (calendar_file, listing_file) in zip(calendar_files, listing_files):
    ...
    # Rename "San Polo" and "Santa Croce" with "San Polo Santa Croce"
    df['neighbourhood_cleansed'] = df['neighbourhood_cleansed'].replace({'San Polo': 'San Polo Santa Croce', 'Santa Croce': 'San Polo Santa Croce'})
...
# Only keep Neighbourhoods San Polo Santa Croce
calendar_summary_all = calendar_summary_all[calendar_summary_all['neighbourhood_cleansed'].isin(['San Polo Santa Croce'])]

# Save final summary
calendar_summary_all.to_csv("airbnb_data/merged_calendar_summary_external.csv", index=False)
